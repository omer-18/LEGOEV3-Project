...
void driveDist_CM_ENC(int dist_cm, int motor_power);

bool obstacleDetected();	// Returns a boolean if a object is detected
...
task main()
{
	...
	bool green = false;
	bool detect;
	int middle_ENC = 0;	// Initializing the location of the drivable area
	int around = 0;
...
{
	double dist_enc = dist_cm * CM_TO_ENC;
	drive(motor_power);

	while(nMotorEncoder[motorB] < dist_enc)
		{}
	drive(0);
}
...
bool obstacleDetected()
{
	...
	while (initial_height - SensorValue[S2] < 2.0 && SensorValue[S3] != (int)colorGreen)
	{
